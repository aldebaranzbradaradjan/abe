...
pub fn create_register_email(mail: &str, username: &str, token: &str) -> Result<SendableEmail, ApiError> {
    let (content, alternative) = template::mail::register_user(mail, username, token)?;
    Ok(SendableEmail {
        to: mail.into(),
...
pub fn create_reset_token_email(
    ...
    let (content, alternative) = template::mail::reset_token(mail, username, token)?;
    Ok(SendableEmail {
        to: mail.into(),
...
pub fn create_password_changed_success_email(
    ...
    let (content, alternative) = template::mail::change_password_success(mail, username)?;
    Ok(SendableEmail {
        to: mail.into(),
